...
1. _Editing files._ Using `nano`{.bash} (or your linux editor of choice like `emacs`{.bash} or `vi`{.bash}), create a file called `aboutme.txt` in your new `ling334/hw1/` directory. In this file, answer the following questions about yourself:
...
5. _Regular expressions in python._ Create `problem5.py` in `ling334/hw1/`. Have the script go through each line of the brown corpus (the single file mentioned above), check whether each line contains at least 3 capitalized words (using a single regular expression), and write those lines that do to a file called `browncaps.txt` in `ling334/hw1/`. To verify that your script works, run it from the terminal and then look at `browncaps.txt` to verify its output.
